...
import ra.model.serviceImple.UserService;
...
public class PasswordResetTokenController {
    ...
    public ResponseEntity<?> resetPassword(@RequestParam("email") String userEmail, HttpServletRequest request) {
        Users user = (Users) userService.findUsersByUserName(userEmail);
        if (userService.existsByEmail(userEmail)) {
            ...
            return new ResponseEntity<>(new MessageResponse("Email is not already"), HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<?> creatNewPass(@RequestParam("token") String token, @RequestParam("newPassword") String newPassword) {
        ...
        long date1 = passwordResetToken.getStartDate().toEpochDay() + 1800000;
